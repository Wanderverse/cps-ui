 ...
 * By default, this script will look for a script ending in shoelace.js or coaxium-autoloader.js and set the base path
  ...
  if (!basePath) {
    ...
    if (configScript) {
      ...
      const fallbackScript = scripts.find(s => {
        return /shoelace(\.min)?\.js($|\?)/.test(s.src) || /coaxium-autoloader(\.min)?\.js($|\?)/.test(s.src);
      });
      let path = '';
